        ...
        <div className="div1">
          ...
          <div className="basket-product<">
            ...
            <div className="buy-product">
              ...
              {this.state.basket.map((item,i) => <p name={this.state.basket[i]} key={i}>{this.state.basket[i]} {this.state.basketPrice[i]}€
               <button className="product-remove" onClick={event => this.removeProduct(event,i, this.state.basket[i],this.state.basketPrice[i])}>X</button>
                </p>
              )}
            ...
            <div className="total-price">
              <div className="wrapper">
                <button className="commande-button">
                  <a href="/form">
                  Commander {this.state.totalPrice}€<span></span>
                  <span></span>
                  <span></span>
                  <span></span>
                  </a>
        ...
        {csv.map((data, key) => {
          return (
            <div className="product" key={key}>
              <div className="">
                <img className="img-product" alt={data.nom} src={data.url} title={data.prix+"€"} ></img>
                
              </div>
              <button className="add-product" onClick={event => this.handleClick(event, data.nom,data.prix)}>
                    Ajouter {data.prix}€
                </button>
            </div>
